...
class ChatArchiveStore(SchemaMixin, PendingWalMixin):
    ...
    def _message_text_from_raw(self, raw: Any) -> str:
        if isinstance(raw, str):
            parsed = self._json_loads_maybe(raw)
            return self._message_text_from_raw(parsed) if parsed is not None else raw

        parts: list[str] = []
        for element in self._raw_message_elements(raw):
            text = self._message_element_text(element)
            if text:
                parts.append(text)
        if parts:
            return "".join(parts)
    ...
    def _message_element_text(self, element: dict[str, Any]) -> str:
        ...
        segment = self._onebot_segment(element)
        if segment:
            kind, data = segment
            return self._segment_text(kind, data)

        text_element = element.get("textElement")
        if isinstance(text_element, dict):
            content = self._first_text(text_element, ("content", "text"))
            at_type = str(text_element.get("atType") or "").lower()
            if at_type in {"1", "all", "at_all", "mention_all"}:
                return content or "@全体成员"
            if at_type in {"2", "one", "at", "mention"}:
                return content or self._mention_text(text_element)
            return content

        for key in ("atElement", "mentionElement"):
            if isinstance(element.get(key), dict):
                return self._mention_text(element[key])

        for key in ("faceElement", "marketFaceElement", "mfaceElement", "market_face"):
            if isinstance(element.get(key), dict):
                return self._first_text(
                    element[key],
                    ("faceName", "face_name", "faceText", "summary", "name"),
                )
        ...
        if isinstance(reply, dict):
            return self._first_text(
                reply,
                ("sourceMsgText", "source_msg_text", "text", "summary", "content"),
            )

        ark = element.get("arkElement") or element.get("ark")
        if isinstance(ark, dict):
            parsed = self._json_loads_maybe(
                ark.get("bytesData") or ark.get("data") or ark.get("arkData")
            )
            if isinstance(parsed, dict):
                return self._first_text(parsed, ("prompt", "summary", "title", "desc"))

        kind = self._infer_forward_segment_kind(element)
        return self._segment_text(kind, element)

    def _segment_text(self, kind: str, data: dict[str, Any]) -> str:
        normalized = str(kind or "").strip().lower()
        if normalized in {"text", "plain"}:
            return self._first_text(data, ("text", "content", "value"))
        if normalized in {"mention_all", "at_all"}:
            return "@全体成员"
        if normalized in {"at", "mention"}:
            return self._mention_text(data)
        if normalized in {"face", "mface", "marketface", "market_face"}:
            return self._first_text(
                data, ("summary", "faceName", "face_name", "name", "faceText")
            )
        if normalized in {"image", "img", "pic", "picture"}:
            return self._first_text(data, ("summary", "name", "file", "file_name"))
        if normalized in {"video", "record", "audio", "voice", "ptt", "file"}:
            return self._first_text(
        ...
        if normalized in {"json", "light_app"}:
            parsed = self._json_loads_maybe(
                data.get("data") or data.get("json_payload")
            )
            if isinstance(parsed, dict):
                return self._first_text(parsed, ("prompt", "summary", "title", "desc"))
        return self._first_text(data, ("summary", "text", "content", "title", "name"))

    def _mention_text(self, data: dict[str, Any]) -> str:
        qq = self._first_text(
            data,
            (
                "qq",
        ...
        name = self._first_text(data, ("name", "nick", "nickname", "content", "text"))
        if qq == "all" or str(data.get("type") or "").lower() in {
            "mention_all",
            "at_all",
        }:
            return "@全体成员"
        return f"@{name or qq or '成员'}"
    ...
    def _onebot_forward_archive(self, data: dict[str, Any]) -> dict[str, Any] | None:
        ...
        if not forward_id:
            return None
        preview_values = data.get("preview")
        preview = (
            [str(item) for item in preview_values if str(item or "").strip()]
        ...
        return self._build_forward_archive(
            ...
            title=self._first_text(data, ("title", "name")) or "合并转发",
            summary=self._first_text(data, ("summary", "desc", "description"))
            or "OneBot 合并转发",
            previews=preview,
            messages=messages,
    ...
    def _normalize_media_kind(kind: Any) -> str:
        ...
        if (
            ...
            or "image" in value
            or "pic" in value
            or value in {"mface", "marketface", "market_face", "sticker"}
            or "market_face" in value
            or "marketface" in value
            or "mface" in value
        ):
            return "image"
