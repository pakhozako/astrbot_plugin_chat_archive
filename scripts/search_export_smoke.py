...
class DummyEvent:
    def __init__(
        ...
        *,
        ...
        self.unified_msg_origin = "search-export:test:room"
        self.platform_meta = SimpleNamespace(name="search-export")
        self.message_obj = SimpleNamespace(
            ...
            session_id="search-export-session",
...
async def seed_store(root: Path) -> ChatArchiveStore:
    ...
    image.write_bytes(b"search-export-image")
    now = int(time.time())
...
async def smoke_search_and_filters(root: Path) -> None:
    store = await seed_store(root / "filters")
    ...
    assert any(row["sender_name"] == "Alice" for row in suggestions["senders"]), (
        suggestions
    )
    assert any(row["value"] == "group" for row in suggestions["message_types"]), (
        suggestions
    )
    assert any(row["value"] == "image" for row in suggestions["media_kinds"]), (
        suggestions
    )
    await store.close()


async def smoke_exports(root: Path) -> None:
    ...
    with zipfile.ZipFile(zip_path) as zf:
        ...
        assert "messages.json" in names, names
        assert "messages.md" in names, names
        assert any(
            name.startswith("media/") and name.endswith(".png") for name in names
        ), names

    export_file = store.get_export_file(json_path.name)
    assert export_file and export_file["path"] == json_path.resolve(), export_file
    assert store.get_export_file(str(json_path)) is None
    assert store.get_export_file("../chat_archive.sqlite3") is None
...
async def main() -> None:
    root = Path(tempfile.mkdtemp(prefix="search_export_"))
    try:
        await smoke_search_and_filters(root)
        await smoke_exports(root)
        print("search export smoke OK")
    finally:
