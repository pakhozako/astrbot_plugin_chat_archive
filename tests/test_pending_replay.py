...
async def make_store(data_dir: Path, *, batch_size: int = 20, interval: float = 3600) -> ChatArchiveStore:
    return ChatArchiveStore(
        data_dir,
        ArchiveConfig(batch_size=batch_size, flush_interval_seconds=interval, durable_write=True),
    )
