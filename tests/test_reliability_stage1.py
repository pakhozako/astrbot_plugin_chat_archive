...
async def test_media_gc_ref_count_and_orphan_file(root: Path) -> None:
    ...
    with sqlite3.connect(data_dir / "chat_archive.sqlite3") as conn:
        ...
        uid = conn.execute("SELECT message_uid FROM messages WHERE message_id = ?", ("media-1",)).fetchone()["message_uid"]
    ...
    with sqlite3.connect(data_dir / "chat_archive.sqlite3") as conn:
        ...
        uid = conn.execute("SELECT message_uid FROM messages WHERE message_id = ?", ("media-2",)).fetchone()["message_uid"]
