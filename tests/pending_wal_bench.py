...
def pending_lines(store: ChatArchiveStore) -> int:
    if not store.pending_path.exists():
        return 0
    return len([line for line in store.pending_path.read_text(encoding="utf-8").splitlines() if line.strip()])
...
async def run_kill_28() -> dict:
    ...
    try:
        ...
        for i in range(1, 29):
            await store.store_event(DummyEvent(f"kill-{i}", f"kill scenario message {i}"))
        before = {
...
async def run_perf(count: int, *, durable_write: bool, simulate_without_pending_wal: bool = False) -> float:
    root = Path(tempfile.mkdtemp(prefix="pending_perf_"))
    try:
        store = ChatArchiveStore(
            root,
            ArchiveConfig(batch_size=20, flush_interval_seconds=3600, durable_write=durable_write),
        )
...
async def main() -> None:
    ...
    for count in (100, 500, 1000):
        before_wal = await run_perf(count, durable_write=False, simulate_without_pending_wal=True)
